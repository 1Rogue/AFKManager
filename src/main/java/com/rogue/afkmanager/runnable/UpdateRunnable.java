...
public class UpdateRunnable extends BukkitRunnable {
    ...
    private static final String VERSION_URL = "https://raw.github.com/1Rogue/AFKManager/master/VERSION";
    ...
    public UpdateRunnable() {
        super();
        version = AFKManager.getPlugin().getDescription().getVersion();
    }

    public void run() {
        if (version.endsWith("SNAPSHOT") || version.endsWith("DEV")) {
            AFKManager.getPlugin().getLogger().warning("You are using a dev build, update checks are disabled");
            isLatest = true;
            return;
        }
        try {
            ...
            AFKManager.getPlugin().getLogger().log(Level.SEVERE, "Error occured while checking for an update", ex);
